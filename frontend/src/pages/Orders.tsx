...
export const Orders = () => {
  ...
  useEffect(() => {
    void ordersApi.list().then((data) => setOrders(data.orders));
  }, []);
            ...
            <tbody className="divide-y divide-white/5">
              {filteredOrders.length === 0 ? (
                ...
                filteredOrders.map((order, i) => (
                  <motion.tr key={order.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="hover:bg-white/[0.02] transition-colors group">
