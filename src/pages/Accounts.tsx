...
export const Accounts = () => {
  ...
  useEffect(() => {
    ...
    const q = query(
      collection(db, 'operatorAccounts'), 
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setAccounts(accs);
    }, (err) => handleFirestoreError(err, OperationType.LIST, 'operatorAccounts'));
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      await deleteDoc(doc(db, 'operatorAccounts', id));
    } catch (err) {
      handleFirestoreError(err, OperationType.DELETE, `operatorAccounts/${id}`);
    }
  };
