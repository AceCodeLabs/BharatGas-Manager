...
import { ShoppingCart, CheckCircle, Clock, AlertCircle, Package, TrendingUp } from 'lucide-react';
...
export const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const unsubAccounts = onSnapshot(query(collection(db, 'operatorAccounts'), where('ownerId', '==', user.uid)), (snap) => {
        setStats(prev => prev.map(s => s.label === 'Total Sync Accounts' ? { ...s, value: snap.size.toString() } : s));
        setStats(prev => prev.map(s => s.label === 'Active Sessions' ? { ...s, value: snap.docs.filter(d => d.data().status === 'active').length.toString() } : s));
    });

    const unsubOrders = onSnapshot(query(collection(db, 'orders'), where('ownerId', '==', user.uid)), (snap) => {
        const docs = snap.docs.map(d => d.data());
        setStats(prev => prev.map(s => s.label === 'Pending Orders' ? { ...s, value: docs.filter(d => d.status === 'pending').length.toString() } : s));
        setStats(prev => prev.map(s => s.label === 'Completed' ? { ...s, value: docs.filter(d => d.status === 'completed').length.toString() } : s));
        
        const sorted = snap.docs
          .map(doc => ({ id: doc.id, ...doc.data() }))
          .sort((a: any, b: any) => (b.orderId || 0) - (a.orderId || 0))
          .slice(0, 4);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto min-h-screen bg-[#0a0a0a]">
      ...
      <header className="mb-10">
        ...
        <p className="text-gray-500 text-sm">Real-time overview of your delivery operations</p>
      </header>
        ...
        {stats.map((stat, i) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest bg-white/5 px-2 py-1 rounded">Live</div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 pb-12">
        <div className="bg-[#141414] rounded-[2rem] border border-white/5 overflow-hidden">
            <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between">
                ...
                <button className="text-xs font-bold text-green-500 hover:opacity-80 transition-opacity">View All</button>
            </div>
