...
import { X, ChevronDown, Loader2, KeyRound } from 'lucide-react';
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      if (otp === '123456' && (whitelistCode === 'BG-998' || !whitelistCode)) {
        try {
          await addDoc(collection(db, 'operatorAccounts'), {
            mobile,
            name: mobile === '8799190172' ? 'PRAGTI OFFICE' : 'NEW OPERATOR',
            deviceId: 'dev-' + Math.random().toString(36).slice(2, 9),
            model: 'Generic Android',
            status: 'Active',
            ownerId: user.uid,
            whitelistCode: whitelistCode || 'BG-998',
          ...
          handleFirestoreError(err, OperationType.CREATE, 'operatorAccounts');
        } finally {
          setLoading(false);
        }
      } else {
        setError('Invalid OTP or Whitelist code');
        setLoading(false);
      }
    }, 1500);
  };
